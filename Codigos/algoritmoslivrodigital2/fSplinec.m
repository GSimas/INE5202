...
%1a. eq. i=2
t(2)=0; r(2)=2*h(1)+2*h(2);d(2)=h(2);b(2)=6*((y(3)-y(2))/h(2)-(y(2)-y(1))/h(1))-h(1)*u; %para S1
for i=3:m-1
    t(i)=h(i-1);r(i)=2*(h(i-1)+h(i)); d(i)=h(i);b(i)=6*((y(i+1)-y(i))/h(i)-(y(i)-y(i-1))/h(i-1)); %para Si
end%for
%ultima eq. i=m
t(m)=h(m-1);r(m)=(2*h(m-1)+2*h(m)); d(m)=0;b(m)=6*((y(m+1)-y(m))/h(m)-(y(m)-y(m-1))/h(m-1))-h(m)*v; %para Sm
